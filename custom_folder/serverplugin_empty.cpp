...
inline int IndexOfEdict(const edict_t *pEdicts)
{
	return (int)(pEdicts - gpGlobals->pEdicts);
}
...
public:
	...
	virtual void			ServerActivate( edict_t *pEdictsList, int edictCount, int clientMax );
	...
	virtual void			ClientSettingsChanged( edict_t *pEdicts );
...
bool CEmptyServerPlugin::Load(	CreateInterfaceFn interfaceFactory, CreateInterfaceFn gameServerFactory )
{
	...
	Warning("MEU PLUGIN CARREGOU O LOAD!\n");
	...
	gameeventmanager = (IGameEventManager2 *)interfaceFactory(INTERFACEVERSION_GAMEEVENTSMANAGER,NULL);
...
//---------------------------------------------------------------------------------
void CEmptyServerPlugin::ServerActivate( edict_t *pEdictsList, int edictCount, int clientMax )
{
}
...
// Purpose: called on level start
//---------------------------------------------------------------------------------
void CEmptyServerPlugin::ClientSettingsChanged( edict_t *pEdicts )
{
	if ( playerinfomanager )
	{
		IPlayerInfo *playerinfo = playerinfomanager->GetPlayerInfo( pEdicts );

		const char * name = engine->GetClientConVarValue( IndexOfEdict(pEdicts), "name" );
													   ...
													   // OR if you are accessing the player before they are fully connected
		{
			...
			engine->ClientPrintf( pEdicts, msg ); // this is the bad way to check this, the better option it to listen for the "player_changename" event in FireGameEvent()
...
CON_COMMAND( DoAskConnect, "Server plugin example of using the ask connect dialog" )
{
	...
	else
	{
		...
		{
			edict_t *pEdicts = PEntityOfEntIndex( i );
			if ( pEdicts )
			{
				helpers->CreateMessage( pEdicts, DIALOG_ASKCONNECT, kv, &g_EmtpyServerPlugin );
			}
		}
